...
class MujocoModel(object):
    ...
    """
    #ANDYTODO: this could probably be sped up by caching names but is probably not worth it
    def correct_naming(self, names):
        ...
        if type(names) is str:
            return ''.join([self.naming_prefix, names]) if not self.exclude_from_prefixing(names) else names
        elif type(names) is list:
            return [''.join([self.naming_prefix, name]) if not self.exclude_from_prefixing(name) else name for name in names]

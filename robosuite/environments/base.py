...
class MujocoEnv(metaclass=EnvMeta):
    ...
    Args:
        ...
        init_paused (bool): If True, start the environment in a paused state (press SPACE to unpause)
    ...
    def __init__(
        ...
        self._default_action = np.zeros(self.action_dim, dtype=np.float) if default_action is None else default_action
        self._prev_action = self._default_action
        self._action_q = deque()
    ...
    def set_policy_lag(self, lag: int):
        self._policy_lag = lag
    ...
    def _reset_internal(self):
        ...
        if self.has_renderer and self.viewer is None:
            self.viewer = MujocoPyRenderer(self.sim, init_paused=self.init_paused)
    ...
    def step(self, action):
        ...
        # append this action to the action queue
        self._action_q.append([self._policy_lag, np.copy(action)])
        ...
        for _ in range(int(self.control_timestep / self.model_timestep)):
            policy_step = (len(self._action_q) > 0) and (self._action_q[0][0] == 0)
            
            if policy_step:
              _, a = self._action_q.popleft()
              self._prev_action = a
            else:
              a = self._prev_action
            
            self.sim.forward()
            self._pre_action(a, policy_step)
            self.sim.step()
            self._update_observables()
        
            for qa in self._action_q:
                qa[0] -= 1

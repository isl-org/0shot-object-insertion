...
def simple_axangle2quat(axis, angle_deg):
    """
    quat from unit vector axis and rotation angle angle_deg
    """
    q = np.zeros(4)
...
def clip_axangle_rotation(rot, limit_deg):
    a_deg = np.rad2deg(np.linalg.norm(rot))
    if a_deg > limit_deg:
        return rot*limit_deg/a_deg, True
    else:
        return rot, False

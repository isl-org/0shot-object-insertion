...
class Controller(object, metaclass=abc.ABCMeta):
    ...
    def update(self, force=False):
        ...
        if self.new_update or force:
            #TODO: I think this is unnecessary.  This is the first step of step(), so let's remove it for now.
            #self.sim.forward()
    ...
    @property
    def constraint_compensation(self):
        """
        Constraint compensation for this robot arm (weight of the grasped object)

        Returns:
            np.array: torques
        """
        return self.sim.data.qfrc_constraint[self.qvel_index]

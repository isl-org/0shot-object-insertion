...
class SingleArm(Manipulator):
    ...
    def __init__(
        ...
        self.initialized_control_limits = False #lazy evaluation
    ...
    def control(self, action, policy_step=False):
        ...
        if policy_step:
            self.controller.set_goal(arm_action) #ANDYNOTE: This is going to run update, and run_controller will
        ...
        # if np.logical_or(torques<low, torques>high).any():
        #     print('Torque clipped')
        self.torques = np.minimum(np.maximum(torques, low, out=torques), high, out=torques)
    ...
    def action_limits(self):
        ...
        #ANDYNOTE: I assume that output is always a 1-D array to speed this up.
        #This could possibly be generalized but may accrue some additional overhead
        #for dynamically figuring out loop structure

        if self.has_gripper:

            if not self.initialized_control_limits:
                gripper_dof_len = self.gripper.dof
                self.other_control_len = self.controller.control_limits[0].shape[0]
                out_dim = gripper_dof_len + self.other_control_len

                self.low = np.zeros((out_dim,))
                self.high = np.zeros((out_dim,))

                self.initialized_control_limits = True

                #moved this into the if block.  Assumes a static morphology.
                self.low[:self.other_control_len] = self.controller.control_limits[0]
                self.high[:self.other_control_len] = self.controller.control_limits[1]
                self.low[self.other_control_len:] = -1
                self.high[self.other_control_len:] = 1
        else:
            self.low, self.high = self.controller.control_limits
